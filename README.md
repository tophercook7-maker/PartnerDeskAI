...
- **Autonomous Lead Pipeline** (v8.4) — Logan Leads now supports a controlled lead pipeline: find, qualify, prepare outreach, mark sent, schedule follow-ups, and retire dead leads. Logan does not send messages automatically; Topher approves and sends outreach manually. New per-lead fields (`email`, `evidence`, `outreach_status`, `outreach_subject`, `outreach_body`, `next_follow_up_at`, `follow_up_count`, `dead_reason`, …) live alongside the v6.9/v7.x lead schema — additive, not replacing. New endpoints: `POST /api/leads/{id}/prepare-outreach` (generates the email subject + body locally — no OpenAI), `/mark-sent` (records you sent it manually + schedules a 3-day follow-up), `/write-follow-up`, `/snooze`, `/dead`. The dead-reason picker validates against an enum (`GET /api/leads/dead-reasons`).
- **YouTube Growth Partner** (v8.5) — a 4th agent (alongside Parker/Logan/Olivia) for approval-based YouTube content packages: ideas, scripts, Shorts, thumbnails, metadata, and full packages. Generation runs from local templates only — **NO OpenAI, NO YouTube API, NO uploads**. Every output saves as `draft` and shows a "Review required before publishing" note. Channel Profile (niche, audience, tone, offer, length, focus) persists to `data/youtube_channel.json` (gitignored); packages to `data/youtube_packages.json` (gitignored). Status lifecycle: `draft → approved → used`. New `Connected Accounts` panel inside the section is future-ready — 6 platforms (YouTube/Facebook/Instagram/LinkedIn/TikTok/X) render as `Not connected · Coming soon`; nothing is wired to any social API yet.
- **Video Partner** (v8.6) — a 5th agent, sibling to YouTube Growth Partner, focused on local-business video creation + marketing campaigns across platforms. Generators: 30-day video idea calendar, short-form script, local business ad script, shot list, caption pack, title/description/hashtags pack, and full campaign package (composed of all of the above for one topic). Same safety perimeter as v8.5: **NO OpenAI, NO YouTube/TikTok/Instagram/Facebook API, NO uploads**. Every package saves as `draft` with a "Review required before publishing. This tool prepares content only." disclaimer baked into the body. Profile (business name, niche, target customer, offer, tone, platforms, video length, CTA) persists to `data/video_profile.json` (gitignored); packages to `data/video_packages.json` (gitignored). Same `draft → approved → used` lifecycle, same Output History UI as YouTube Growth Partner.

## Recent versions

**v8.0 — Hub Simplification Pass** *(current)*. The Hub was reorganized into a `Today / Parker / Logan / Olivia / System` hierarchy with a sticky top bar and collapsible partner sections. First-time users should grok the page in under 30 seconds; existing element ids and endpoints unchanged.

The v7.x series (v7.1 → v7.31) layered the Logan lead pipeline, scout queue, lead dashboard with click-to-filter, drag-and-drop pipeline board, multi-template outreach messaging, bulk publish, generation-time platform filtering, and partner-card honesty fixes onto the Hub.
